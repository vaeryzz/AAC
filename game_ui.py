...
class LoadGame:
	...
	def load_file(self, *args):
		...
		if btn_txt == "Load" and filename != "":
			...
			try:
				self.world.go_place._leave_place()
			except:
				pass
	...
	def load_data(self, data, *args):
		...
		* second separator
		...
		self.world.perks = copy.deepcopy(gd.perks)
		self.perks = self.world.perks
		self.world.aktivql = []
		self.world.fertigql = []
		try: self.world.go_perks = None  # attempt to reset
		except: pass
...
class SaveGame:
	...
	def add_save_option(self, *args):
		...
		nm = self.pdata["name"]
		lvl = self.pdata["stats"][5] // 1000
		text = " New ->  {};{}_{};{};{}_{};{};{}".format(nm, lvl, m, d, y, h, mn, s)
...
class Dialogs:
	...
	def load_dialog_ui(self, *args):
		...
		for i in range(len(self.tdl)):
			...
			if cdata[0] == "place":  # place
				...
				command = lambda _=1, i=i: self._menu_screen(i)
	...
	def _menu_screen(self, *args):
		self._leave_dlg()
		self.world.start_screen()
...
class Perks:
	...
	def reset_perks(self, *args):
		self.leave_perks_ui()  # TEST 9/29/2018
