...
class AAB:
	...
	def game_menu(self, *args):
		...
		self.cn.tag_lower(self._m("menu_icons"))	
	...
	def start_screen_exit(self, stage=0, *args):
		...
		for i in tags:
			self.cn.tag_raise(self._m(i))
		self._uistatus("inaktiv")
		#self.start_quest() replace with dialog
		'''
		if stage == 0:
			self._uistatus("aktiv")
	...
	def load_basic_ui(self, *args):
		...
		bilist = ["bpack", "book", "scroll", "settings"]
		for i in range(len(bilist)):
			n = (i+1)*-1
			img = "bw-i_{}".format(bilist[n])
			x, y = 755+(i*-45), 5
			self._mimg(x, y, img, ("mi_"+bilist[n], "menu_icons"))
		self.cn.tag_bind(self._m("mi_book"), "<Button-1>", lambda _=1: self.start_perks())
		self.cn.tag_bind(self._m("mi_bpack"), "<Button-1>", lambda _=1: self.start_player_inv())
		self.cn.tag_bind(self._m("mi_scroll"), "<Button-1>", self.start_quest)
	...
	def load_game_data(self, *args):
		...
		self.perks = copy.deepcopy(game_data.perks)
		self.quests = game_data.quests
		self.aktivql = ["MQ00"]  # current active quests
			...
			if paths != None:
				if "Victory" in self.battle_result:
					...
					if x <= 5:
						...
						self.characters[player]["stats"][0][0] = 2
					else:
						#self._mimg(0, 0, "aBg1")
						self.start_dialog(paths[1])
						self.characters[player]["stats"][0][0] = 1
